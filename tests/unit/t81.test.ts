...
describe("t81 aidlc-state practices-event — bolt-plan-marker-conflict override (migrated from t81-bolt-plan-override.sh, plan 4)", () => {
  ...
  test("3: framework event count pinned at 102", () => {
    ...
    // CEREMONY_SET (per-intent ceremony settings) = 99; GUARD_POLICY_SET (the
    // Guard Policy setting row that replaces CHANGE_CONTROL_SET, which stays
    // registered as a read-only legacy row) + GUARD_RESTORED (the per-run fence
    // switch going back on) + GUARD_STOOD_ASIDE (a fence that stood aside under
    // a lowered policy word or a per-run switch) (+3) = 102.
